...
/// A real planet seed whose far shore is reached by all THREE strands (faith +
/// colony + sword) — seed 27, shore "Zuk". After the naming re-calibration
/// (MIN_CONTINENT_DIVISOR 40→100 for the periodic planet), the periodic planet's
/// medium continents are named again, so the three carriers' shores overlap and a
/// natural three-strand far shore occurs once more. This test proves the marquee
/// chronicle actually OCCURS in a world generated by the binary (not just in the
/// drift-immune SYNTHETIC weave-logic fixture in `mapgen-lore/tests/shore.rs`),
/// and survives the generate → gzip → read-back → weave round-trip. Behavioral
/// seed: if a future history-sim change shifts seed 27's strand mix, re-derive
/// (the synthetic shore.rs test still guards the weave logic regardless).
const THREE_STRAND_SEED: &str = "27";
...
fn lore_auto_shore_weaves_the_far_shore_chronicle_through_the_cli() {
    ...
    // seed 27 reaches its shore by faith, colony, AND the sword — each beat NAMING
    // the shore, and the name reaches the page ONLY because the weaver read the
    // `far_shore` tags (no event summary carries it), proven here to survive the
    // generate → gzip → read-back → weave CLI round-trip. The marquee three-strand
    // chronicle, occurring in a real generated world again after the naming
    // re-calibration.
    for beat in [
