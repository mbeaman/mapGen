...
/// Inner content of `<g class="layer-{name}" ...>…</g>`, tolerating extra attrs
/// (e.g. `display="none"`) on the open tag and balancing nested `<g>`.
fn layer_group<'a>(svg: &'a str, name: &str) -> &'a str {
    let open = format!(r#"<g class="layer-{name}""#);
    let start = svg
        .find(&open)
        .unwrap_or_else(|| panic!("no <g class=\"layer-{name}\"> group in render"));
