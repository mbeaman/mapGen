...
Set `GenerateParams::planet` → `periodic:true`. Re-anchored `seed9_planet_full` and its wasm twin (native↔wasm re-verified 5/5). Re-derived from a CALIBRATED probe (it reproduced the old flat constants exactly before being trusted on periodic worlds): CROSSING `[11,19,7,4]→[11,19,26,30]`, SUNDERED `[23,42]→[23,10]` (42 became a crossing seed), COLONIZE `[2,5,9,11,18]→[18,27,32]`. The marquee 3-strand far shore briefly stopped occurring naturally (no seed in 0..200 put faith+colony+sword on one shore) — root cause: the FLAT-tuned naming threshold (`MIN_CONTINENT_DIVISOR=40` ≈ 2.5%) under-named the periodic planet's smaller continents, and carriers tag only NAMED shores. **RECOVERED by re-calibrating naming 40→100 (≈1%)** — a fix justified on its own (the periodic planet was under-labeled), with the chronicle returning as a consequence: seed 27 reaches shore "Zuk" by all three strands, proven end-to-end in `lore_cli`. `shore.rs`'s weave LOGIC stays pinned by a drift-immune SYNTHETIC fixture (the 3-strand is rare under periodicity, ~6 seeds/200); `colony_far_shore_claims.rs` guards the colony tag on a real world; a calibration guard (`continents_spec`) pins the threshold. Also moved fixtures whose premise changed: faith_crossing/shore laneless → a SUNDERED seed, RIVER_SEED 11→26, trade_prosperity 19→11.
