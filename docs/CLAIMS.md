...
> **Trade & embargo surfacing — DONE in Phase 3 (below).** Capacity/population were SimState-internal; Phase 3 added the `Nation::prosperity` channel (v21) + a prosperity heatmap, a trade-route lens, chronicle weaving, (increment 2) the **first-contact arc** — the narrator weaves a sea route's birth AND its severance into one Sundered-Lane chronicle — and (increment 3) the **landmass place tag** — inter-continental events now carry the far continent they reached (`Event::far_shore`, v22), and a faith's first water-crossing (`FaithCrossed`) is a chronicle that NAMES the shore it touched. The remaining arc work weaves all strands of a far shore (trade, faith, sword — realm tagging + the beachhead tag deferred to it) and temporal surfacing of the diffusion/faith timeline.
