...
> **Trade & embargo surfacing — DONE in Phase 3 (below).** Capacity/population were SimState-internal; Phase 3 added the `Nation::prosperity` channel (v21) + a prosperity heatmap, a trade-route lens, chronicle weaving, (increment 2) the **first-contact arc** — the narrator weaves a sea route's birth AND its severance into one Sundered-Lane chronicle — (increment 3) the **landmass place tag** — inter-continental events now carry the far continent they reached (`Event::far_shore`, v22), and a faith's first water-crossing (`FaithCrossed`) is a chronicle that NAMES the shore it touched — and (increment 4) the **multi-strand far-shore weave** — a colony (`CityFounded`) and a beachhead conquest (`Siege`) now carry `far_shore` too, and the `auto-shore` chronicle weaves EVERY strand that reached one landmass (faith, colony, sword) into a single chronicle naming the shore. The remaining arc work is realm tagging (`far_realms`, still no consumer), the trade-route strand (architecturally out — lore can't map a realm to a continent at load time), and temporal surfacing of the diffusion/faith timeline.
