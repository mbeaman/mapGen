...
> **Trade surfacing — deferred to Phase 3.** Capacity / population are SimState-internal (not serialized), so trade's "realms grow" effect has no `WorldData` render channel yet; the persistent trace is the `TradeRouteOpened` events (which feed the future chronicle). A prosperity overlay + narrative surfacing lands with Phase 3. Remaining Phase-2 deferral: **embargoes** (`EmbargoImposed` on hostiles).
