...
| Latest commit | **Relief R3 (shipped + reviewed + cleaned) + R4 PARTIAL** (2026-06-11): R3 = the oblique-camera increment (tilt, pitch-aware near, oblique LOD, patches-first pick) + review fixes (live terrain-clearance ceiling; gesture-leak guard) + cleanup. R4 = the quality hunt MEASURED a 0.177 seam-shade residual (visible line at every seam) and BUILT the fix (lambert zeroes the cross-edge gradient at tile boundaries → bit-identical; golden re-anchored + wasm twin) + the texBytes→gpuBytes rename. R4 TOUCHES Rust (mapgen-render + golden). Commits `b906ad8`→`f38691e`→`c30e9c6`→`60d44d7`→R4. NEXT: R4 close-out (screenshots + frustum selector, needs a GPU) — see BACKLOG "Up next". |
